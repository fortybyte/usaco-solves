...
void solve() {
	...
	for (int i = 0; i < d; i ++) {
		...
		cin >> a >> b >> c;
		a--; b--;
		flights[a].push_back(makep(b,c));
	}
	vvll mn(n,{MAXINT,MAXINT});
	mn[0] = {0,0};
	...
	while (!pq.empty()) {
		node cur = pq.top();
		pq.pop();
		ll cc= mn[cur.pos][cur.use];
		if (cc != cur.w) continue;
		if (cur.pos == n-1) break; // done with all nodes
		for (auto p : flights[cur.pos]) {
			if (!cur.use) {
				ll temp = cc + p.s/2;
				//cout << p.f << endl;
				if (temp < mn[p.f][1]) {
					 mn[p.f][1] = temp; //<------- here
					 pq.push({p.f,1,temp}); //<----- here
				}
			}
			if (cc + p.s < mn[p.f][cur.use]) {
				mn[p.f][cur.use] = cc + p.s;// <----- here
				pq.push({p.f,cur.use,mn[p.f][cur.use]});// <----- here
			}
		}
	}
	//OUTV2(mn)
